...
from monitoring.dashboard import get_dashboard, update_bot_status
from feedback.learning_engine import get_learning_engine
from monitoring.trade_db import save_trade as save_trade_db
load_dotenv()

# Dedicated trade log file (rotates daily)
logger.add(
    ...
    rotation="00:00",
    retention="30 days",
    filter=lambda r: r["extra"].get("category") == "trade",
)
...
class IntegratedBTCStrategy(Strategy):
    ...
    def __init__(self, redis_client=None, enable_grafana=True, test_mode=False):
        ...
        # Web Dashboard (always on, port 3000)
        self.dashboard = get_dashboard(port=3000)
            ...
            if sim_mode is not None:
                ...
                if redis_simulation != self.current_simulation_mode:
                    ...
                    if self.dashboard_started:
                        update_bot_status(mode="test" if self.test_mode else ("simulation" if redis_simulation else "live"))
    ...
    def on_start(self):
        ...
        if not self.dashboard_started:
            ...
            update_bot_status(
                mode="test" if self.test_mode else ("live" if not self.current_simulation_mode else "simulation"),
    ...
    def on_quote_tick(self, tick: QuoteTick):
        ...
        try:
            ...
            # Push real-time status to dashboard (~every 10 ticks)
            if self.dashboard_started and getattr(self, '_tick_counter', 0) % 10 == 0:
                slug = ""
                ...
                update_bot_status(
                    ...
                    current_market=slug,
                    total_ticks=getattr(self, '_tick_counter', 0),
                )
    ...
    async def _record_paper_trade(self, signal, position_size, current_price, direction):
        ...
        # Push trade info to dashboard
        if self.dashboard_started:
            update_bot_status(last_trade_at=datetime.now(timezone.utc).isoformat())

        # Save to SQLite and trade log
        try:
            slug = ""
            if 0 <= self.current_instrument_index < len(self.all_btc_instruments):
                slug = self.all_btc_instruments[self.current_instrument_index].get("slug", "")
            ...
            logger.bind(category="trade").info(
            ...
            logger.warning(f"Failed to save trade to DB/log: {e}")
...
def run_integrated_bot(simulation: bool = False, enable_grafana: bool = True, test_mode: bool = False):
    ...
    # In simulation mode, skip the exec client entirely (no need to authenticate
    # with Polymarket API credentials for paper trading).
    if simulation:
        config = TradingNodeConfig(
            ...
            # No exec_clients — paper trades are local, no real order submission
        )
    else:
        config = TradingNodeConfig(
            environment="live",
